...
async function detectNode(root: string): Promise<DetectionResult> {
  ...
  try {
    ...
    // Detect paths
    const source = findNodeSourcePaths(root, pkg);
    const tests = findNodeTestPaths(root, pkg);
 ...
 */
function detectNodeFramework(pkg: any): string | undefined {
 ...
 */
function detectNodeDatabase(pkg: any): { type: string; orm?: string } | undefined {
  const deps = { ...pkg.dependencies, ...pkg.devDependencies };
 ...
 */
function findNodeSourcePaths(root: string, pkg: any): string[] {
  const paths: string[] = [];
 ...
 */
function findNodeTestPaths(root: string, pkg: any): string[] {
  const paths: string[] = [];
