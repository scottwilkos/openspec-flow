...
export function generateConfig(
  ...
  const pathsConfig = generatePathsConfig(detection, options.projectName);
 ...
 */
function generateTechStackConfig(detection: DetectionResult): Record<string, any> {
  ...
  if (detection.runtime.language) {
    config.runtime.language = detection.runtime.language;
    if (detection.runtime.languageVersion) {
      config.runtime.language_version = detection.runtime.languageVersion;
    }
  }
...
function generatePathsConfig(
  ...
  if (detection.paths.source.length > 0) {
    config.source.core = detection.paths.source[0];
    ...
    if (detection.runtime.type === 'dotnet') {
      ...
      config.source.domain = `${mainSource}/Domain`;
      config.source.application = `${mainSource}/Application`;
      config.source.infrastructure = `${mainSource}/Infrastructure`;
      config.source.endpoints = `${mainSource}/Endpoints`;
    }
  }
  ...
  if (detection.paths.tests.length > 0) {
    config.tests.unit = detection.paths.tests[0];
    if (detection.paths.tests.length > 1) {
      config.tests.integration = detection.paths.tests[1];
    }
  }
